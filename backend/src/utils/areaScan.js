...
/**
 * Scans a bounded numeric range of candidate pincodes under each given
 * prefix, aggregating every post office India Post returns for them
 * (optionally filtered to one exact district name). This is a best-effort
 * sample, not an exhaustive directory — the public postal API only supports
 * exact 6-digit lookups, so full coverage of a city/state would require
 * many thousands of calls. Results are deduplicated by pincode+name.
 */
async function scanAreas({ prefixes, from = 1, to = 150, filterDistrict = null, concurrency = 15 }) {
  const candidates = [];
  ...
  const perPincode = await mapWithConcurrency(candidates, concurrency, async (pincode) => {
    ...
    const filtered = filterDistrict
      ...
      : localities;
    return filtered.map((l) => ({ name: l.name.trim(), pincode }));
  });
